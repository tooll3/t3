...
namespace T3.Operators.Types.Id_7814fd81_b8d0_4edf_b828_5165f5657344
{
    public class AnimVec3 : Instance<AnimVec3>
    {
        ...
        private void Update(EvaluationContext context)
        {
            ...
            var timeZ = (time + phases.Z) * masterRate * rateFactorFromContext * rates.Z;

            Result.Value 
                = new Vector3( AnimMath.CalcValueForNormalizedTime(shape, timeX, 0) * amplitudes.X + offsets.X,
                               AnimMath.CalcValueForNormalizedTime(shape, timeY, 1) * amplitudes.Y + offsets.Y,
                               AnimMath.CalcValueForNormalizedTime(shape, timeZ, 2) * amplitudes.Z + offsets.Z);
        }
