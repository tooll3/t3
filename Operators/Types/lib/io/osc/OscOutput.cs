...
namespace T3.Operators.Types.Id_4e99da86_482f_4037_8664_b2371526d632
{
    public class OscOutput : Instance<OscOutput>, IStatusProvider
    {
        ...
        public readonly InputSlot<string> Location = new();

        [Input(Guid = "d5e9e9be-093b-4d57-9070-9b2cf33aa45b")]
        public readonly InputSlot<string> String = new();

        [Input(Guid = "827c7ae2-c129-4d04-a715-328c0a86bf8a")]
        public readonly InputSlot<float> Number = new();
        ...
        private void Update(EvaluationContext context)
        {
            ...
            var invalidMessage = $"Failed to build the OSC message({oscType})";
            
            switch (oscType)
            {
                case OscTypes.Number:
                    if (Math.Abs(num - _numericValue) > 0.0001f)
                    {
                        var message = new OscMessage(location, num);
                        ...
                        else _lastErrorMessage = $"{invalidMessage}: {message.ErrorMessage}";
                    }
                    break;
