...
namespace Spout.Std
{
    namespace BasicString
    {
        ...
        public unsafe partial struct __Internalc__N_std_S_basic_string__C___N_std_S_char_traits__C___N_std_S_allocator__C
        {
            ...
            [SuppressUnmanagedCodeSecurity, DllImport("Spout.Std-symbols", EntryPoint = "??0?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@std@@QEAA@XZ", CallingConvention = __CallingConvention.Cdecl)]
            ...
            [SuppressUnmanagedCodeSecurity, DllImport("Spout.Std-symbols", EntryPoint = "??1?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@std@@QEAA@XZ", CallingConvention = __CallingConvention.Cdecl)]
    ...
    // DEBUG:     }
    // DEBUG: 
    // DEBUG:     _CONSTEXPR20 void _Modify_annotation(const difference_type _Count) const noexcept {
    // DEBUG:         // Extends/shrinks the annotated range by _Count
    // DEBUG:         if (_Count == 0) {
    // DEBUG:             return;
    // DEBUG:         }
    // DEBUG: 
    // DEBUG:         auto& _My_data = _Mypair._Myval2;
    // DEBUG:         _Apply_annotation(
    // DEBUG:             _My_data._Myptr(), _My_data._Myres, _My_data._Mysize, static_cast<size_type>(_My_data._Mysize + _Count));
    // DEBUG:     }
    // DEBUG: 
    // DEBUG:     _NODISCARD static const void* _Get_aligned_first(const void* _First, const size_type _Capacity) noexcept {
    // DEBUG:         const char* _CFirst = reinterpret_cast<const char*>(_First);
    // DEBUG: 
    // DEBUG:         if (_Capacity >= _Asan_granularity) { // We are guaranteed to have sufficient space to find an aligned address
    // DEBUG:             return reinterpret_cast<const void*>(
    // DEBUG:                 (reinterpret_cast<uintptr_t>(_CFirst) + (_Asan_granularity - 1)) & ~(_Asan_granularity - 1));
    // DEBUG:         }
    // DEBUG: 
    // DEBUG:         uintptr_t _Alignment_offset = reinterpret_cast<uintptr_t>(_CFirst) & (_Asan_granularity - 1);
    // DEBUG:         if (_Alignment_offset != 0) {
    // DEBUG:             _Alignment_offset = _Asan_granularity - _Alignment_offset;
    // DEBUG:         }
    // DEBUG: 
    // DEBUG:         if (_Capacity > _Alignment_offset) {
    // DEBUG:             return _CFirst + _Alignment_offset;
    // DEBUG:         }
    // DEBUG: 
    // DEBUG:         return nullptr;
    // DEBUG:     }
    // DEBUG: 
    // DEBUG:     static _CONSTEXPR20 void _Apply_annotation(const value_type* _Ptr, const size_type _Capacity,
    ...
    // DEBUG: 
    // DEBUG:         if (!_Asan_string_should_annotate) {
    // DEBUG:             return;
    // DEBUG:         }
    // DEBUG: 
    // DEBUG:         // We need to check whether we have a misaligned SSO buffer because of the proxy in `_Container_base` (e.g. x86)
    // DEBUG:         if constexpr (_Memcpy_val_offset % _Asan_granularity != 0) {
    // DEBUG:             const uintptr_t _Alignment_offset = reinterpret_cast<uintptr_t>(_Ptr) & (_Asan_granularity - 1);
    // DEBUG:             if (_Alignment_offset != 0 && _Capacity == _BUF_SIZE - 1) {
    // DEBUG:                 return;
    // DEBUG:             }
    // DEBUG:         }
    // DEBUG: 
    // DEBUG:         // Needs to consider the null terminator
    // DEBUG:         const char* _First    = reinterpret_cast<const char*>(_Ptr);
    // DEBUG:         const char* _End      = reinterpret_cast<const char*>(_Ptr + _Capacity + 1);
    // DEBUG:         const char* _Old_last = reinterpret_cast<const char*>(_Ptr + _Old_size + 1);
    // DEBUG:         const char* _New_last = reinterpret_cast<const char*>(_Ptr + _New_size + 1);
    // DEBUG:         if constexpr (_Has_minimum_allocation_alignment_string<basic_string>) {
    // DEBUG:             __sanitizer_annotate_contiguous_container(_First, _End, _Old_last, _New_last);
    // DEBUG:         } else {
    // DEBUG:             const void* _Aligned_first = _Get_aligned_first(_First, _Capacity + 1);
    // DEBUG:             if (!_Aligned_first) {
    // DEBUG:                 // There is no aligned address within the underlying buffer. Nothing to do
    // DEBUG:                 return;
    // DEBUG:             }
    // DEBUG: 
    // DEBUG:             const void* _Aligned_old_last = _Old_last < _Aligned_first ? _Aligned_first : _Old_last;
    // DEBUG:             const void* _Aligned_new_last = _New_last < _Aligned_first ? _Aligned_first : _New_last;
    // DEBUG:             const void* _Aligned_end      = _End < _Aligned_first ? _Aligned_first : _End;
    // DEBUG:             __sanitizer_annotate_contiguous_container(
    // DEBUG:                 _Aligned_first, _Aligned_end, _Aligned_old_last, _Aligned_new_last);
    // DEBUG:         }
    // DEBUG:     }
    // DEBUG: 
    // DEBUG: #define _ASAN_STRING_MODIFY(n)    _Modify_annotation((n))
    ...
    // DEBUG: #else // ^^^ _INSERT_STRING_ANNOTATION ^^^ // vvv !_INSERT_STRING_ANNOTATION vvv
    // DEBUG: #define _ASAN_STRING_MODIFY(n)
    ...
    // DEBUG:         auto _ULast  = _Get_unwrapped(_Last);
    ...
    // DEBUG:                 _Construct<_Construct_strategy::_From_ptr>(
    // DEBUG:                     _UFirst, _Convert_size<size_type>(static_cast<size_t>(_ULast - _UFirst)));
    // DEBUG:             } else {
    ...
    // DEBUG: #if _HAS_CXX20
    // DEBUG:         if (_STD is_constant_evaluated()) {
    // DEBUG:             _My_data._Myres = _BUF_SIZE; // TRANSITION: constexpr SSO
    // DEBUG:         }
    // DEBUG: 
    // DEBUG:         const bool _Stay_small = _Count < _BUF_SIZE && !_STD is_constant_evaluated();
    // DEBUG: #else // ^^^ _HAS_CXX20 / !_HAS_CXX20 vvv
    // DEBUG:         const bool _Stay_small = _Count < _BUF_SIZE;
    // DEBUG: #endif // _HAS_CXX20
    // DEBUG: 
    // DEBUG:         if (_Stay_small) {
    // DEBUG: #if _HAS_CXX20
    // DEBUG:             // TRANSITION: This is currently unused until SSO support is merged
    // DEBUG:             if (_STD is_constant_evaluated()) {
    // DEBUG:                 _Construct_in_place(_My_data._Bx);
    // DEBUG:             }
    // DEBUG: #endif // _HAS_CXX20
    // DEBUG: 
    ...
    // DEBUG:                 _Traits::move(_My_data._Bx._Buf, _Arg, _Count);
    // DEBUG:                 _Traits::assign(_My_data._Bx._Buf[_Count], _Elem());
    // DEBUG:             } else { // _Strat == _Construct_strategy::_From_string
    // DEBUG: #ifdef _INSERT_STRING_ANNOTATION
    // DEBUG:                 _Traits::move(_My_data._Bx._Buf, _Arg, _Count);
    // DEBUG: #else // ^^^ _INSERT_STRING_ANNOTATION ^^^ // vvv !_INSERT_STRING_ANNOTATION vvv
    // DEBUG:                 _Traits::move(_My_data._Bx._Buf, _Arg, _BUF_SIZE);
    // DEBUG: #endif // !_INSERT_STRING_ANNOTATION
    // DEBUG:             }
    // DEBUG: 
    ...
    // DEBUG: 
    // DEBUG:         _My_data._Myres               = _BUF_SIZE - 1;
    // DEBUG:         const size_type _New_capacity = _Calculate_growth(_Count);
    ...
    // DEBUG: 
    // DEBUG:     template <class _Iter>
    // DEBUG:     _CONSTEXPR20 void _Construct_from_iter(_Iter _First, const _Iter _Last) {
    // DEBUG:         auto& _My_data  = _Mypair._Myval2;
    // DEBUG:         auto& _Al       = _Getal();
    // DEBUG:         auto&& _Alproxy = _GET_PROXY_ALLOCATOR(_Alty, _Al);
    // DEBUG:         _Container_proxy_ptr<_Alty> _Proxy(_Alproxy, _My_data);
    ...
    // DEBUG:             const auto _Count = _Convert_size<size_type>(static_cast<size_t>(_STD distance(_First, _Last)));
    // DEBUG:             if (_Count > max_size()) {
    // DEBUG:                 _Xlen_string(); // result too long
    // DEBUG:             }
    // DEBUG: 
    // DEBUG: #if _HAS_CXX20
    // DEBUG:             const bool _Become_large = _Count >= _BUF_SIZE || _STD is_constant_evaluated();
    // DEBUG: #else // ^^^ _HAS_CXX20 / !_HAS_CXX20 vvv
    // DEBUG:             const bool _Become_large = _Count >= _BUF_SIZE;
    // DEBUG: #endif // _HAS_CXX20
    // DEBUG:             if (_Become_large) {
    // DEBUG:                 const size_type _New_capacity = _Calculate_growth(_Count);
    // DEBUG:                 const pointer _New_ptr        = _Al.allocate(_New_capacity + 1); // throws
    // DEBUG:                 _Construct_in_place(_My_data._Bx._Ptr, _New_ptr);
    // DEBUG:                 _My_data._Myres = _New_capacity;
    ...
    // DEBUG:                     }
    // DEBUG: 
    // DEBUG:                     const auto _Old_ptr           = _My_data._Myptr();
    // DEBUG:                     const size_type _New_capacity = _Calculate_growth(_My_data._Mysize);
    // DEBUG:                     const pointer _New_ptr        = _Al.allocate(_New_capacity + 1); // throws
    // DEBUG: 
    // DEBUG: #if _HAS_CXX20
    // DEBUG:                     if (_STD is_constant_evaluated()) { // Begin the lifetimes of the objects before copying to avoid UB
    ...
    // DEBUG:                     if (_My_data._Myres >= _BUF_SIZE) { // Need to deallocate old storage
    ...
    // DEBUG:             _ASAN_STRING_MODIFY(static_cast<difference_type>(_Right_size));
    ...
    // DEBUG:             _ASAN_STRING_MODIFY(static_cast<difference_type>(_Left_size));
    ...
    // DEBUG:         auto& _My_data = _Mypair._Myval2;
    // DEBUG:         _STL_INTERNAL_CHECK(!_My_data._Large_string_engaged() && _My_data._Mysize == 0);
    // DEBUG:         _STL_INTERNAL_CHECK(_Size < _Res); // So there is room for null terminator
    // DEBUG:         _Traits::assign(_Right[_Size], _Elem());
    // DEBUG: 
    // DEBUG:         const bool _Is_large = _Res > _BUF_SIZE; // Note: _BUF_SIZE because _Res now includes the null terminator
    // DEBUG:         if (_Is_large) {
    ...
    // DEBUG:             _ASAN_STRING_CREATE(*this);
    // DEBUG:         } else {
    // DEBUG:             _ASAN_STRING_MODIFY(static_cast<difference_type>(_Size));
    ...
    // DEBUG: #ifdef _INSERT_STRING_ANNOTATION
    // DEBUG:                 if (!_Right_data._Large_string_engaged()) {
    // DEBUG:                     _ASAN_STRING_REMOVE(_Right);
    // DEBUG:                 }
    // DEBUG: #endif // _INSERT_STRING_ANNOTATION
    // DEBUG: 
    ...
    // DEBUG: #if _HAS_CXX20
    // DEBUG:             if (_STD is_constant_evaluated()) { // begin the lifetime of the array elements before copying into them
    // DEBUG:                 _Construct_in_place(_Mypair._Myval2._Bx);
    // DEBUG:             }
    // DEBUG: #endif // _HAS_CXX20
    ...
    // DEBUG: 
    // DEBUG: private:
    // DEBUG:     _CONSTEXPR20 void _Copy_assign_val_from_small(const basic_string& _Right) {
    ...
    // DEBUG:         _ASAN_STRING_MODIFY(static_cast<difference_type>(1 - _Mypair._Myval2._Mysize));
    ...
    // DEBUG:             _ASAN_STRING_MODIFY(static_cast<difference_type>(_Count));
    ...
    // DEBUG:             _ASAN_STRING_MODIFY(static_cast<difference_type>(_Count - _Mypair._Myval2._Mysize));
    // DEBUG:             _Elem* const _Old_ptr   = _Mypair._Myval2._Myptr();
    ...
    // DEBUG: 
    // DEBUG: #if _HAS_CXX20
    // DEBUG:         const bool _Do_become_small = _My_data._Mysize < _BUF_SIZE && !_STD is_constant_evaluated();
    // DEBUG: #else // ^^^ _HAS_CXX20 / !_HAS_CXX20 vvv
    // DEBUG:         const bool _Do_become_small = _My_data._Mysize < _BUF_SIZE;
    // DEBUG: #endif // _HAS_CXX20
    // DEBUG:         if (_Do_become_small) {
    // DEBUG:             _Become_small();
    // DEBUG:             return;
    // DEBUG:         }
    // DEBUG: 
    ...
    // DEBUG: #if _HAS_CXX20
    // DEBUG:         if (_STD is_constant_evaluated()) {
    // DEBUG:             _Construct_in_place(_Starts_large._Bx);
    // DEBUG:         }
    // DEBUG: #endif // _HAS_CXX20
    ...
    // DEBUG: #ifdef _INSERT_STRING_ANNOTATION
    // DEBUG:         if (_My_large && _Right_large) {
    // DEBUG:             // nothing
    // DEBUG:         } else if (_My_large) {
    // DEBUG:             _ASAN_STRING_REMOVE(_Right);
    // DEBUG:         } else if (_Right_large) {
    // DEBUG:             _ASAN_STRING_REMOVE(*this);
    // DEBUG:         } else {
    // DEBUG:             _ASAN_STRING_REMOVE(_Right);
    // DEBUG:             _ASAN_STRING_REMOVE(*this);
    ...
    // DEBUG:                     _ASAN_STRING_CREATE(*this);
    // DEBUG:                 } else if (_Right_large) {
    // DEBUG:                     _ASAN_STRING_CREATE(_Right);
    // DEBUG:                 } else {
    // DEBUG:                     _ASAN_STRING_CREATE(_Right);
    // DEBUG:                     _ASAN_STRING_CREATE(*this);
    // DEBUG:                 }
    ...
    // DEBUG:     }
    // DEBUG: 
    // DEBUG:     _CONSTEXPR20 void _Become_small() {
    // DEBUG:         // release any held storage and return to small string mode
    // DEBUG:         auto& _My_data = _Mypair._Myval2;
    // DEBUG:         _STL_INTERNAL_CHECK(_My_data._Large_string_engaged());
    // DEBUG:         _STL_INTERNAL_CHECK(_My_data._Mysize < _BUF_SIZE);
    // DEBUG: 
    // DEBUG:         _My_data._Orphan_all();
    // DEBUG:         _ASAN_STRING_REMOVE(*this);
    // DEBUG:         const pointer _Ptr = _My_data._Bx._Ptr;
    // DEBUG:         auto& _Al          = _Getal();
    // DEBUG:         _Destroy_in_place(_My_data._Bx._Ptr);
    // DEBUG: #if _HAS_CXX20
    // DEBUG:         if (_STD is_constant_evaluated()) { // begin the lifetime of the array elements before copying into them
    // DEBUG:             _Construct_in_place(_Mypair._Myval2._Bx);
    // DEBUG:         }
    ...
    // DEBUG:         _Al.deallocate(_Ptr, _My_data._Myres + 1);
    // DEBUG:         _My_data._Myres = _BUF_SIZE - 1;
    // DEBUG:         _ASAN_STRING_CREATE(*this);
    // DEBUG:     }
    // DEBUG: 
    // DEBUG:     _CONSTEXPR20 void _Eos(const size_type _New_size) { // set new length and null terminator
    // DEBUG:         _ASAN_STRING_MODIFY(static_cast<difference_type>(_New_size - _Mypair._Myval2._Mysize));
    ...
    public unsafe partial class BasicString<_Elem, _Traits, _Alloc> : IDisposable
    {
        ...
        internal static readonly global::System.Collections.Concurrent.ConcurrentDictionary<IntPtr, global::Spout.Std.BasicString<_Elem, _Traits, _Alloc>> NativeToManagedMap = new global::System.Collections.Concurrent.ConcurrentDictionary<IntPtr, global::Spout.Std.BasicString<_Elem, _Traits, _Alloc>>();
    ...
    public unsafe partial class CharTraits<_Elem> : IDisposable
    {
        ...
        internal static readonly global::System.Collections.Concurrent.ConcurrentDictionary<IntPtr, global::Spout.Std.CharTraits<_Elem>> NativeToManagedMap = new global::System.Collections.Concurrent.ConcurrentDictionary<IntPtr, global::Spout.Std.CharTraits<_Elem>>();
    ...
    public unsafe static partial class BasicStringExtensions
    {
        public partial struct __Internal
        {
            [SuppressUnmanagedCodeSecurity, DllImport("Spout.Std-symbols", EntryPoint = "?assign@?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@std@@QEAAAEAV12@QEBD@Z", CallingConvention = __CallingConvention.Cdecl)]
            ...
            [SuppressUnmanagedCodeSecurity, DllImport("Spout.Std-symbols", EntryPoint = "?data@?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@std@@QEBAPEBDXZ", CallingConvention = __CallingConvention.Cdecl)]
        ...
        public static global::Spout.Std.BasicString<sbyte, global::Spout.Std.CharTraits<sbyte>, global::Spout.Std.Allocator<sbyte>> Assign(this global::Spout.Std.BasicString<sbyte, global::Spout.Std.CharTraits<sbyte>, global::Spout.Std.Allocator<sbyte>> @this, string _Ptr)
        {
            ...
            var __result0 = global::Spout.Std.BasicString<sbyte, global::Spout.Std.CharTraits<sbyte>, global::Spout.Std.Allocator<sbyte>>.__GetOrCreateInstance(__ret, false);
            return __result0;
        }
...
namespace Spout.Std
{
    ...
    namespace Allocator
    {
        ...
        public unsafe partial struct __Internal
        {
            [SuppressUnmanagedCodeSecurity, DllImport("Spout.Std-symbols", EntryPoint = "??0?$allocator@D@std@@QEAA@XZ", CallingConvention = __CallingConvention.Cdecl)]
    ...
    public unsafe partial class Allocator<_Ty> : IDisposable
    {
        ...
        internal static readonly global::System.Collections.Concurrent.ConcurrentDictionary<IntPtr, global::Spout.Std.Allocator<_Ty>> NativeToManagedMap = new global::System.Collections.Concurrent.ConcurrentDictionary<IntPtr, global::Spout.Std.Allocator<_Ty>>();
