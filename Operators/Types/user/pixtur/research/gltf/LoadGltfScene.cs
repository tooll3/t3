                           ...
                           , IDescriptiveFilename, IStatusProvider
{
    ...
    private void ParseChildren(IEnumerable<Node> visualChildren, SceneSetup.SceneNode parentNode)
    {
        ...
        {
            if(!TryGenerateMeshBuffersFromGltfChild(child, out var meshBuffers, out var errorMessage))
            {
            ...
            var sceneNode = new SceneSetup.SceneNode()
                                {
                                    Name = child.Name,
            ...
            parentNode.ChildNodes.Add(sceneNode);
            ParseChildren(child.VisualChildren, sceneNode);
        }
    }
    

    private bool TryGenerateMeshBuffersFromGltfChild(Node child, out MeshBuffers newMesh, out string message)
    {
