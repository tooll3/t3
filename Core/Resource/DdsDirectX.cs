            ...
            switch (resDim)
            {
                case ResourceDimension.Texture1D:
                {
                    var desc = new Texture1DDescription()
                        ...
                        var dataBoxes = new SharpDX.DataBox[initData.Length];
                        for (var dataIndex = 0; dataIndex < initData.Length; dataIndex++)
                        {
                            dataBoxes[dataIndex] = new SharpDX.DataBox
                                                       {
                                                           DataPointer = initData[0].Data,
                                                           RowPitch = (int)initData[0].Pitch,
                                                           SlicePitch = (int)initData[0].SlicePitch
                                                       };
                        }
                        
                        //texture = device.CreateTexture1D(desc, initData);
                        texture = new Texture1D(device, desc, dataBoxes);
                ...
                case ResourceDimension.Texture2D:
                {
                    var desc = new Texture2DDescription
                        ...
                        texture = new Texture2D(device, desc, initData);
