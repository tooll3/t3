...
namespace T3.Editor.Gui.Interaction.TransformGizmos
{
    ...
    static class TransformGizmoHandling
    {
        ...
        static void UpdateInternalState()
        {
            ...
            var yaw = MathUtils.ToRad * r.Y;
            var pitch = MathUtils.ToRad * r.X;
            var roll = MathUtils.ToRad * r.Z;

            var c = Vector3.TransformNormal(new Vector3(t.X, t.Y, t.Z), _objectToWorld);
            _selectedCenter = new Vector3(c.X, c.Y, c.Z);

            _localToObject = T3.Core.Utils.Math3DUtils.CreateTransformationMatrix(scalingCenter: Vector3.Zero,
            ...
            Vector4 originInClipSpace = Vector4.Transform(new Vector4(t.X, t.Y, t.Z, 1), _objectToClipSpace);
        ...
        static bool HandleDragOnAxis(Vector3 gizmoAxis, Color color, GizmoParts mode)
        {
                     ...
                     && _dragInteractionWindowId == ImGui.GetID(""))
            {
                ...
                rayInLocal.Position = Vector3.Transform(rayInObject.Position, _initialObjectToLocal);
                ...
                if (_plane.Normal != Vector3.Zero
                    && _plane.Intersects(in rayInLocal, out Vector3 intersectionPoint))
                {
        ...
        private static void StartPositionDragging(GizmoParts mode, bool createUndo)
        {
            ...
            else if (!_plane.Intersects(in rayInLocal, out _initialIntersectionPoint))
            {
                _plane.D = -_plane.D;
                if (!_plane.Intersects(in rayInLocal, out _initialIntersectionPoint))
                {
                    _plane.Normal = Vector3.Zero;
        ...
        private static void UpdatePositionDragging(Vector3 newOrigin)
        {
        ...
        private static Ray GetPickRayInObject(Vector2 posInScreen)
        {
            ...
            var rayStartInObject = Vector3.Transform(rayStartInClipSpace, clipSpaceToObject);

            var rayEndInClipSpace = new Vector3(xInClipSpace, yInClipSpace, 1);
            var rayEndInObject = Vector3.Transform(rayEndInClipSpace, clipSpaceToObject);
        ...
        private static float CalcGizmoScale(EvaluationContext context, Matrix4x4 localToObject, float width, float height, float fovInDegree,
                                            float gizmoSize)
        {
        ...
        private static Plane GetPlaneForDragMode(GizmoParts mode, Vector3 normDir, Vector3 origin)
        {
            switch (mode)
            {
                ...
                {
                    var secondAxis = Math.Abs(Vector3.Dot(normDir, Vector3.UnitY)) <
                                     Math.Abs(Vector3.Dot(normDir, Vector3.UnitZ))
                                         ? Vector3.UnitY
                                         : Vector3.UnitZ;
                    return System.Numerics.Plane.CreateFromVertices(origin, origin + Vector3.UnitX, origin + secondAxis);
                }
                case GizmoParts.PositionYAxis:
                {
                    var secondAxis = Math.Abs(Vector3.Dot(normDir, Vector3.UnitX)) <
                                     Math.Abs(Vector3.Dot(normDir, Vector3.UnitZ))
                                         ? Vector3.UnitX
                                         : Vector3.UnitZ;
                    return Plane.CreateFromVertices(origin, origin + Vector3.UnitY, origin + secondAxis);
                }
                case GizmoParts.PositionZAxis:
                {
                    var secondAxis = Math.Abs(Vector3.Dot(normDir, Vector3.UnitX)) <
                                     Math.Abs(Vector3.Dot(normDir, Vector3.UnitY))
                                         ? Vector3.UnitX
                                         : Vector3.UnitY;
                    return Plane.CreateFromVertices(origin, origin + Vector3.UnitZ, origin + secondAxis);
                }
                case GizmoParts.PositionOnXyPlane:
                    return Plane.CreateFromVertices(origin, origin + Vector3.UnitX, origin + Vector3.UnitY);

                case GizmoParts.PositionOnXzPlane:
                    return Plane.CreateFromVertices(origin, origin + Vector3.UnitX, origin + Vector3.UnitZ);

                case GizmoParts.PositionOnYzPlane:
                    return System.Numerics.Plane.CreateFromVertices(origin, origin + Vector3.UnitY, origin + Vector3.UnitZ);
            }
